 ...
 * 
 */
__global__ void compute_shortest_path(int num_cities, int *dists, int *mins) {
    int shortest_length = INT_MAX;
    int num_threads = blockIdx.x;
    int tid = threadIdx.x;
    long max_check = factorial(num_cities);
    long tours_to_check = max_check / num_threads;
    int *current = kth_perm((tid+1)*(int)tours_to_check, (int)num_cities);
    ...
    do {
        ...
        at++;
    } while (at < (int)((tid+1)*tours_to_check));
...
int main(int argc, char **argv) {
    ...
    //permutations
    if(num_threads > fact) {
        fprintf(stderr, "Too many processors to run effectively... refactoring number of threads.\n");
    ...
    //print_tsp(h_distances, num_cities, random_seed);
